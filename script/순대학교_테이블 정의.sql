...
CREATE TABLE STUDENT (
	...
	DEPT_NO	VARCHAR2(30)	NOT NULL,
	S_EDATE	DATE DEFAULT SYSDATE NOT NULL ,
	S_ABSENCE	CHAR(2)	CHECK ( S_ABSENCE IN ('Y','N')) NOT NULL ,
	S_PHONE     VARCHAR2(20),
	S_EMAIL     VARCHAR2(20),
	S_IMG   VARCHAR2(1000),
	S_STATUS CHAR(2)  CHECK ( S_STATUS IN ('Y','N'))  NOT NULL
);

--학생 테이블 코멘트
COMMENT ON TABLE STUDENT IS '학생';
COMMENT ON COLUMN STUDENT.S_ID IS '교원번호';
COMMENT ON COLUMN STUDENT.C_NO IS '학번';
COMMENT ON COLUMN STUDENT.C_NO IS '학과번호';
COMMENT ON COLUMN STUDENT.S_PWD IS '비밀번호';
COMMENT ON COLUMN STUDENT.S_NAME IS '학생이름';
COMMENT ON COLUMN STUDENT.DEPT_NO IS '학부 학과테이블 JOIN';
COMMENT ON COLUMN STUDENT.S_EDATE IS '입학일';
COMMENT ON COLUMN STUDENT.S_ABSENCE IS '휴학여부 CHECK IN (Y,N)';
COMMENT ON COLUMN STUDENT.S_PHONE IS '휴대전화';
COMMENT ON COLUMN STUDENT.S_EMAIL IS '휴대전화';
COMMENT ON COLUMN STUDENT.S_IMG IS '사진 경로';
COMMENT ON COLUMN STUDENT.S_STATUS IS '상태CHECK I N (Y | N)';
...
COMMENT ON COLUMN  TIME.T_TIME IS '시간';
...
CREATE TABLE FLUCTUATION (
   ...
   S_ID VARCHAR2(10) NOT NULL,
   FL_CHANGE VARCHAR2(30),
   FL_REASON VARCHAR2(20),
   FL_YEAR   NUMBER   NULL,
...
CREATE TABLE CONSULTING (
   CS_NO VARCHAR2(10) NOT NULL,
   S_ID VARCHAR2(10) NOT NULL,
   CS_CONTENTS   VARCHAR2(30),
   CS_DATE   DATE NOT NULL,
   CS_TIME   VARCHAR2(10) NOT NULL,
...
COMMENT ON COLUMN PROFESSORS.DEPT_NO IS '학부';
COMMENT ON COLUMN PROFESSORS.C_NAME IS '학과';
...
CREATE TABLE CERTIFICATE (
	...
	S_ID	VARCHAR2(10) NOT NULL,	
	CF_NAME	VARCHAR2(30) NOT NULL,	
	CF_ISSUE	VARCHAR2(30) NOT NULL,	
	CF_DATE	DATE	NOT NULL,	
	CF_ATTCHMENT	 VARCHAR2(30),	
	CF_ID	VARCHAR2(30) NOT NULL
);
...
--외래키
ALTER TABLE FLUCTUATION ADD CONSTRAINT FK_STUDENTS_TO_FLUCTUATION_1 FOREIGN KEY (S_ID)REFERENCES STUDENTS (S_ID);
ALTER TABLE CONSULTING ADD CONSTRAINT FK_STUDENTS_TO_CONSULTING_1 FOREIGN KEY (S_ID)REFERENCES STUDENTS (S_ID);
ALTER TABLE CLASS_HISTORY ADD CONSTRAINT FK_STUDENTS_TO_CLASS_HISTORY_1 FOREIGN KEY (S_ID)REFERENCES STUDENTS (S_ID);
ALTER TABLE CLASS_HISTORY ADD CONSTRAINT FK_CLASS_TO_CLASS_HISTORY_1 FOREIGN KEY (class_no)REFERENCES CLASS (class_no);
ALTER TABLE CERTIFICATE ADD CONSTRAINT FK_STUDENTS_TO_CERTIFICATE_1 FOREIGN KEY (S_ID)REFERENCES STUDENTS (S_ID);
