...
--학생전공석차조회 뷰 VIEW_ST_COUNT
CREATE OR REPLACE VIEW VIEW_ST_COUNT AS
SELECT H.S_ID "S_ID", C.CLASS_YEAR "YEAR", C.CLASS_SEMESTER "SEMESTER", ROUND(AVG(NVL(H.CG_POINT,0)),2) "AVG"
FROM CLASS_HISTORY H
JOIN CLASS C ON H.CLASS_NO=C.CLASS_NO
JOIN STUDENT S ON H.S_ID=S.S_ID
GROUP BY C.CLASS_YEAR, C.CLASS_SEMESTER, H.S_ID
ORDER BY H.S_ID, C.CLASS_YEAR, C.CLASS_SEMESTER;

--DROP VIEW VIEW_ST_COUNT;
